document.addEventListener("DOMContentLoaded", function () {
  ...
  scriptTag.src = chrome.runtime.getURL("/provider.js");
...
const onPageMessage = (e) => {
  if (!e.data) return;
  if (
    e.data.type !== "gramWalletAPI_ton_provider_write" &&
    e.data.type !== "gramWalletAPI_ton_provider_connect"
  )
    return;
...
const sendMessageToActivePort = (payload, isRepeat = false) => {
  try {
    ...
    const isInvalidated = err.message
      .toString()
