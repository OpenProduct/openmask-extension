...
const WrapRotorY  = styled(Wrap)<{loading: boolean, left: boolean}>`
    ${props => props.loading && props.left
        ? css`animation: ${loadingLeft} 3s infinite linear;`
        : css`animation: ${loadingRight} 3s infinite linear;`
    }}
`;
...
const bottomFaces = Array.from({length: 8}, (_, i) => styled(TriangleBottom)`
    animation-delay: -${2500 + i * 1000}ms;
    transform: translateY(90px) rotateY(${45 * i}deg) rotateX(35deg) scaleX(.24) scaleY(-1);
`);
...
const middleBottomFaces = Array.from({length: 8}, (_, i) => styled(TriangleMiddleBottom)`
    animation-delay: -${3500 + i * 1000}ms;
    transform: rotateY(${45 * i}deg) translateY(-11px) translateZ(-34px) rotateX(-50deg) scaleX(.24) scaleY(.3);
`);
...
const middleTopFaces = Array.from({length: 8}, (_, i) => styled(TriangleMiddleTop)`
    animation-delay: -${2500 + i * 1000}ms;
    transform: rotateY(${22.5 + 45 * i}deg) translateY(-111px) translateZ(-31px) rotateX(-58deg) scaleX(.13) scaleY(-.36);
`);
...
const upFaces = Array.from({length: 8}, (_, i) => styled(TriangleUp)`
    animation-delay: -${3500 + i * 1000}ms;
    transform: rotateY(${22.5 + 45 * i}deg) translateY(-22px) translateZ(0px) rotateX(-70deg) scaleX(.13) scaleY(.33);
`);
...
export const Logo = () => {
    ...
    >
        <WrapRotorX>
            <WrapRotorY loading={loading} left={left}>
                <WrapRotorZ>
                    {Faces.map((Face, i) => <Face key={'b' + i} />)}
                </WrapRotorZ>
            </WrapRotorY>
        </WrapRotorX>
    </div>;
